fbneo-combo-trials is a framework for fighting game combo trials on the FBNeo emulator used in Fightcade.
Inspiration and large pieces of code taken from peon2's fbneo-training-mode (add link).
...
- Ability to add support for other fighting games. This will take more effort and a basic knowledge of Lua programming. Usually Fightcade's training scripts can be used as a base for memory addresses and basic functions and the like. I'd also suggest looking through the cheat files over at Pugsy's Cheats (add link) and fbneo-cheats (add link) for memory addresses.
...
0. Backup Fightcade's training mode folder in case anything goes wrong. It is located here:
<Fightcade install folder>\emulator\fbneo\fbneo-training-mode
1. Copy the contents of the "statetest" folder to Fightcade's training mode folder. This modifies Fightcade's training mode to display certain relevant information depending on the game.
2. Run Fightcade's training mode with a supported game.
3. Use the provided txt templates to fill out state data for other characters. Don't touch the char_data files unless you're certain of what you're doing - these are used to map out character actions and serve as a foundation for the trials.

Games supported:

Street Fighter Alpha 1 [sfa] - 100% feature complete

Big thanks go out to the Fightcade developers and to the developers of all featured fighting games.

Special thanks go out to the following awesome people:
